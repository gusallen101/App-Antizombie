...
export default function ViewGoalsScreen() {
  ...
  const handleConfirmComplete = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.baseUrl}completar-meta/${goalToComplete.id}`, {
        headers: {
          apikey,
        },
      });
      ...
      Alert.alert(
        t('screens.viewGoals.completeSuccessTitle'),
        t('screens.viewGoals.completeSuccessMessage'),
      );
  ...
  return (
    ...
    >
      <View style={[styles.heroCard, { backgroundColor: palette.surface }]}>
        ...
        <Text style={[styles.heroHeading, { color: palette.textOnSurface }]}>
          {t('screens.viewGoals.heroTitle')}
        </Text>
        <Text style={[styles.heroDescription, { color: palette.inputPlaceholder }]}>{heroDescription}</Text>
      </View>
          ...
          >
            <Text
              style={[
                ...
                { color: category === item.key ? palette.buttonText : palette.textOnSurface },
              ]}
            >
              {t(item.titleKey)}
            </Text>
      ...
      {loading ? (
        <View style={styles.feedbackWrapper}>
          ...
          <Text style={[styles.feedbackLabel, { color: palette.inputPlaceholder }]}>
        ...
        <View style={[styles.feedbackWrapper, { paddingVertical: 48 }]}>
          ...
          <Text style={[styles.feedbackLabel, { color: palette.inputPlaceholder, textAlign: 'center' }]}>
        ...
        goals.map((goal) => (
          <View key={goal.id} style={[styles.goalCard, { backgroundColor: palette.surface }]}>
            <View style={styles.goalHeader}>
              <Text style={[styles.goalTitle, { color: palette.textOnSurface }]}>{goal.etiqueta}</Text>
              ...
              <View style={styles.taskList}>
                ...
                {goal.tasks.map((task, index) => (
                  <View key={`${goal.id}-task-${index}`} style={styles.taskItem}>
                    ...
                    <Text style={[styles.taskLabel, { color: palette.inputPlaceholder }]}>{task}</Text>
                  </View>
                ))}
              </View>
            </View>
            ...
            <View style={[styles.modalCard, { backgroundColor: palette.surface }]}>
              ...
              <ScrollView
                ...
                <View style={[styles.selectedCategoryBadge, { borderColor: palette.border }]}>
                  ...
                  />
                  <Text style={[styles.selectedCategoryLabel, { color: palette.textOnSurface }]}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.confirmCard, { backgroundColor: palette.surface }]}>
            <View style={styles.confirmIcon}>
              <Ionicons name="checkmark-done-outline" size={28} color={palette.primary} />
            </View>
            <Text style={[styles.modalTitle, { color: palette.textOnSurface }]}>
              {t('screens.viewGoals.completeTitle')}
            </Text>
            <Text style={[styles.confirmDescription, { color: palette.inputPlaceholder }]}>
              {t('screens.viewGoals.completeDescription', {
                goal: goalToComplete?.etiqueta ?? '',
              })}
            </Text>
            <View style={styles.modalFooter}>
              <TouchableOpacity
              ...
              >
                <Text style={[styles.cancelLabel, { color: palette.inputPlaceholder }]}>
                  {t('screens.viewGoals.completeCancel')}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.saveButton, { backgroundColor: palette.primary }]}
              ...
              >
                <Text style={[styles.saveLabel, { color: palette.buttonText }]}>
                  {completingGoal ? '…' : t('screens.viewGoals.completeConfirm')}
                </Text>
              </TouchableOpacity>
            </View>
          </View>
