...
export default function FinancialHealthScreen() {
  ...
  const fetchMovements = useCallback(
    async (targetMonth: number, targetYear: number) => {
      try {
        ...
        if (parsed.length === 0 && !refreshing) {
          Alert.alert(
            t('screens.financialHealth.emptyMonthTitle'),
            t('screens.financialHealth.emptyMonthMessage'),
            [
              { text: t('screens.financialHealth.no'), style: 'cancel' },
              {
                text: t('screens.financialHealth.yes'),
                onPress: () => void handleClonePreviousMonth(),
              },
            ]
          );
        }
        ...
        setError(
          caughtError instanceof Error ? translateError(caughtError.message) : t('screens.home.zombieError'),
        );
      } finally {
        setLoading(false);
        setRefreshing(false);
  ...
  const handleClonePreviousMonth = useCallback(async () => {
    try {
      setCloning(true);
      const [apikey, userId] = await Promise.all([
        AsyncStorage.getItem('@auth:apikey'),
      ...
      const response = await fetch(
        `${API_CONFIG.baseUrl}movimientos/${userId}/${prevMonth}/${prevYear}`,
        { headers: { apikey } }
      );

      if (!response.ok) throw new Error('Error al obtener mes anterior');

      const prevMovements = await response.json();

      if (!Array.isArray(prevMovements) || prevMovements.length === 0) {
        Alert.alert(t('screens.financialHealth.noDataTitle'), t('screens.financialHealth.noDataMessage'));
        return;
      }

      // Filtrar solo los fijos (Tipo 1: Gasto Fijo, Tipo 3: Ingreso Fijo)
      const fixedMovements = prevMovements.filter((m: any) => m.tipo === 1 || m.tipo === 3);

      if (fixedMovements.length === 0) {
        Alert.alert(t('screens.financialHealth.noFixedDataTitle'), t('screens.financialHealth.noFixedDataMessage'));
        return;
      }

      // Ejecutar todas las creaciones en paralelo para mayor velocidad
      await Promise.all(fixedMovements.map((m: any) => {
        const params = new URLSearchParams();
        params.append('etiqueta', m.etiqueta);
        params.append('cantidad', String(m.cantidad));
        params.append('tipo', String(m.tipo)); 
        params.append('mes', String(month));
        params.append('anio', String(year));
        params.append('id_usuario', userId);

        return fetch(`${API_CONFIG.baseUrl}movimiento`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
            apikey,
      ...
      Alert.alert(t('screens.financialHealth.copyDone'), t('screens.financialHealth.cloneSuccess'));

      // refrescar
      void fetchMovements(month, year);
    } catch (error) {
      Alert.alert(t('screens.goals.form.errorTitle'), error instanceof Error ? translateError(error.message) : t('screens.home.zombieError'));
    } finally {
      setCloning(false);
    }
  }, [month, year, fetchMovements, translateError]);
  ...
  return (
    ...
    >
      ...
      <View style={[styles.markupCard, { backgroundColor: palette.surface }]}>
        ...
        <Text style={[styles.markupValue, { color: markup >= 0 ? '#4CD964' : '#E67C73', textShadowColor: 'rgba(0,0,0,0.05)', textShadowOffset: {width: 0, height: 2}, textShadowRadius: 4 }]}>
      ...
      {loading ? (
        ...
        <View style={[styles.emptyState, { borderColor: palette.border }]}>
          ...
          <TouchableOpacity 
            style={[styles.cloneButton, { borderColor: palette.primary }]}
            onPress={handleClonePreviousMonth}
            disabled={cloning}
          >
            {cloning ? <ActivityIndicator size="small" color={palette.primary} /> : (
              <Text style={[styles.cloneButtonLabel, { color: palette.primary }]}>{t('screens.financialHealth.cloneButton')}</Text>
            )}
          </TouchableOpacity>
        </View>
      ) : (
